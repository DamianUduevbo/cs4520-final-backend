...
app.get('/recipes', (req : any, res : any) => {
    ...
    if (searchWords) {
        ...
        const filteredRecipes = RecipesDatabase.filter((recipe) => {
            const recipeWords = recipe.name.match(regex);
            return recipeWords?.some((word) => searchWords.includes(word));
        });
        */
...
// create a recipe
app.post('/recipes', (req : any, res : any) => {
    const clientRecipe : ClientRecipe = req.body;
    const recipe : Recipe = {
...
app.post('/users', (req : any, res : any) => {
    const clientUser : ClientUser = req.body;
